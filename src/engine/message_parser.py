...
_continuous_packets: dict[str, bytearray] = {}
_last_src_ack: dict[str, str] = {}


class MessageParser:
    @staticmethod
    def capture(message: str) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            # Fallback for special protocol start
            if len(message) > 100 and message.startswith(PROTOCOL_SIGNATURES["special_start"]):
                ...
                try:
                    ...
                    logger.debug(f"Special message parsing failed: {e}")
                    return None
    ...
    @staticmethod
    def message_to_event(msg_list: list[dict]) -> Optional[list[BaseEvent]]:
        logger = logging.getLogger(LOGGING_NAME)
        try:
            if not msg_list:
                return None
    ...
    @staticmethod
    def identify_event(msg_dict: dict) -> Optional[str]:
    ...
    @staticmethod
    def packet_to_event(packet: Packet) -> Optional[list[BaseEvent]]:
        logger = logging.getLogger(LOGGING_NAME)
        try:
            if IP not in packet or TCP not in packet or not hasattr(packet, "load"):
                return None
            ...
            _continuous_packets.setdefault(packet_key, bytearray())
            _continuous_packets[packet_key].extend(payload)

            if packet_key != _last_src_ack[packet_src]:
                old_key = _last_src_ack[packet_src]
                if old_key in _continuous_packets:
                    data = _continuous_packets.pop(old_key)
                    try:
                        # Decode with ignore to handle partial/corrupt data
                        decoded_load = data.decode('utf-8', errors='ignore')
                        # The game seems to use backslash as a separator in some cases
                        for possible_msg in decoded_load.split("\\"):
                            if not possible_msg:
                                continue
                        ...
                        logger.error(f"Error processing assembled data: {e}")
                else:
