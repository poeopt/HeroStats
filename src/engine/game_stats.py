...
class GameStats:
    ...
    drop_log: DropLog
    zone_eff: ZoneEfficiency
    season_mode: Optional[str]
    death_count: int

    def __init__(self):
        self._log = logging.getLogger(LOGGING_NAME)
        self._mail_cbs: List[Callable[[], None]] = []
        self._init()
    ...
    def on_mail(self, cb: Callable[[], None]):
            ...
            if not was and self.session.has_mail:
                for cb in self._mail_cbs:
                    try:
                        ...
                        self._log.error(f"Error in mail callback: {e}")
    ...
    def update_hourly_stats(self):
        ...
        for rid in ItemsRarity:
            rarity_name = ItemsRarity[rid]
            ph[rarity_name] = self.session.calculate_value_per_hour(
                self.added_items.added_items[rarity_name]['total'])
